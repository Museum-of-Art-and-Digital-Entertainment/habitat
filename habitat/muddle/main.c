  ...
  char	*argv[];
{
	...
	inputClasses = NULL;
	classFile = openFile("class.dat", "w", "class output file");
        classEquatesFile = openFile("class_equates.m", "w",
		"class equates file");
	...
	widthFile = openFile("width.incl.pl1", "w", "pl1 width table file");
	fprintf(widthFile, "/*\n * width.incl.pl1\n");
	fprintf(widthFile, " * Generated by Muddle\n");
	fprintf(widthFile, " * %s */\n", ctime(&theTime));

	pl1File = openFile("capacity.incl.pl1", "w", "pl1 init table file");
	fprintf(pl1File, "/*\n * capacity.incl.pl1\n");
	fprintf(pl1File, " * Generated by Muddle\n");
	fprintf(pl1File, " * %s */\n", ctime(&theTime));

	images.outputFile = openFile("image.dat", "w", "image output file");
	maxItem[(int)IMAGE_ITEM] = images.nextFreeEntry = 0;
	images.name = "Images";

	sounds.outputFile = openFile("sound.dat", "w", "sound output file");
	maxItem[(int)SOUND_ITEM] = sounds.nextFreeEntry = 0;
	sounds.name = "Sounds";

	actions.outputFile=openFile("action.dat", "w", "action output file");
	maxItem[(int)ACTION_ITEM] = actions.nextFreeEntry = 0;
	actions.name = "Actions";

	heads.outputFile = openFile("head.dat", "w", "head output file");
