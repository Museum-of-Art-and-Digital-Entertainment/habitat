...
The behavior code will check to see if your name is on the list,
if it IS NOT, it will add your name to the list. One of the instructions
...
name engraved on this tablet for all
...
2) HIDE it in a place that will make
    it hard to find.
3) SHOW it to NO ONE.
4) HIDE it NOT in a turf.
5) When it is hidden, seek an Oracle
...
Need on host:
behavior for class 97, magic type ?, WIN ROAD RALLY
also class 97, immobile magic item
also add 32 bits of flags to avatar record.
Need on C64:
class 97.
